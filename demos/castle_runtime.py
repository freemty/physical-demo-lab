...
class Run:
    def __init__(self, task, max_steps=8000, dt=1/60):
        ...
        self.app = SimulationApp({
            'headless': True, 'active_gpu': self.args.gpu, 'physics_gpu': 0,
            'multi_gpu': False, 'max_gpu_count': 1, 'renderer': 'RaytracedLighting',
            'width': self.args.width, 'height': self.args.height, 'sync_loads': True,
            'shutdown_watchdog_timeout': 15.0,
            'extra_args': ['--/renderer/multiGpu/activeCudaGpus=0',
                           '--/renderer/multiGpu/useCudaDeviceOrder=true',
                           '--/app/settings/persistent=false', '--/app/asyncRendering=false',
